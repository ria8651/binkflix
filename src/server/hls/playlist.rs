...
/// Render the VOD m3u8. `audio_idx` is appended as a query string to the
/// init.mp4 + segment URIs so hls.js's segment fetches inherit the
/// audio-track selection unambiguously — relative-URI resolution
/// dropping the parent's query is a known footgun.
pub fn render_m3u8(plan: &StreamPlan, audio_idx: u32) -> String {
    ...
    out.push_str(&format!("#EXT-X-MAP:URI=\"init.mp4?a={audio_idx}\"\n"));
    for s in &plan.segments {
        out.push_str(&format!("#EXTINF:{:.3},\n", s.d));
        out.push_str(&format!("seg-{:05}.m4s?a={audio_idx}\n", s.i));
    }
...
mod tests {
    ...
    fn renders_complete_vod_playlist() {
        let s = render_m3u8(&sample_plan(), 0);
